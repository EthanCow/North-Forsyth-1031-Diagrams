<!--
============================================================================
AI EDITOR INSTRUCTIONS  (read first — this block is an HTML comment; it will
not render on the page, but Cursor / any LLM should use it as context)
============================================================================
PURPOSE
  This file is the single source of truth for rebuilding the GitHub Pages site
  "North-Forsyth-1031-Diagrams" around the RECOMMENDED legal structure
  ("Structure A — Exchange First, Then Combine"). It replaces the prior content,
  which described a development-TIC + promote-as-fee + partition-waiver deal that
  a tax review concluded was likely to FAIL the §1031 exchange.

HOW THE SITE WORKS (preserve this convention)
  - The page renders Markdown into TABS. Each tab begins with a marker:
        A tab marker is an HTML comment whose text is "TAB:NAME" (the overview tab, the structure tab, and so on). Keep these markers.
    Keep these markers. The text between two markers is one tab's content.
  - Fenced ```mermaid code blocks render as diagrams (GitHub + most renderers).
    Do NOT HTML-escape <br/> inside ```mermaid fences; literal <br/> is correct
    in a Markdown fence (escaping is only needed inside raw <pre> HTML).
  - Tables are GitHub-flavored Markdown tables.

EDITING RULES
  - Entity names (e.g., "Kao NFCC Owner LLC", "Compass Exchange Services LLC")
    are PLACEHOLDERS. Swap in the real entities when known. Keep the entity TYPE
    and TAX STATUS columns accurate when you rename.
  - Keep the confidentiality / "not a substitute for a signed §1031 opinion"
    disclaimer visible somewhere (footer or a disclaimer tab).
  - Color convention used throughout: Kao = blue, Hanover = amber/orange,
    neutral/third-party = grey, asset/JV = green; tax = green (deferred) /
    red (taxable now) / amber (ordinary income or cost).
  - Numbers are rounded planning figures, not verified deal terms. See
    "Facts & Assumptions" and "Open Items."

WHAT CHANGED VS. THE OLD SITE (one line)
  Old: Kao took a 95% development-TIC + promote-as-fee (fails §1031).
  New: Kao takes 100% fee title in the exchange, then — AFTER A GENUINE
  SEASONING PERIOD — the promoted Hanover JV is formed by a tax-free §721
  contribution (Structure A). Same economics. The exchange is defensible but
  NOT "clean": it is a more-likely-than-not (MLTN) position whose strength
  rises with seasoning. A simpler, stronger fallback (Structure B) is included.

TONE RULES (added after a hostile tax review)
  - Do NOT describe Structure A as "clean," "automatic," or happening
    "immediately after." It is conditional and seasoning-dependent.
  - Do NOT promise a "should" opinion in writing. Target MLTN with a
    documented path to "should."
  - Present the step-transaction risk and the Magneson/Bolker LIMITATIONS
    honestly. Present Structure B as the genuinely stronger exchange.
============================================================================
-->

# North Forsyth §1031 — Structure A ("Exchange First, Then Combine")
...
flowchart LR
  ...
  C["Day 45<br/>Identify NFCC<br/>in writing"]:::neutral
  ...
  G["Phase 2<br/>§721 into PropCo JV<br/>Hanover 5% + promote"]:::han
...
# APPENDICES (deeper context for the AI / for an internal page; not required as a public tab)
